...
```bash
# macOS (Apple Silicon)
curl -L https://github.com/i2y/tsuchi/releases/download/v0.1.0/tsuchi-darwin-aarch64 -o tsuchi
chmod +x tsuchi && mv tsuchi ~/.local/bin/

# Linux (x86_64)
curl -L https://github.com/i2y/tsuchi/releases/download/v0.1.0/tsuchi-linux-x86_64 -o tsuchi
chmod +x tsuchi && mv tsuchi ~/.local/bin/

# Linux (ARM64)
curl -L https://github.com/i2y/tsuchi/releases/download/v0.1.0/tsuchi-linux-aarch64 -o tsuchi
chmod +x tsuchi && mv tsuchi ~/.local/bin/
```
...
```bash
gh release download v0.1.0 --repo i2y/tsuchi --pattern "tsuchi-darwin-aarch64"
chmod +x tsuchi-* && mv tsuchi-* ~/.local/bin/tsuchi
```

### Via pip

```bash
pip install git+https://github.com/i2y/tsuchi.git
```

### Development install

```bash
git clone https://github.com/i2y/tsuchi.git
cd tsuchi
pip install -e .
cd vendor/quickjs-ng && cmake -B build && cmake --build build && cd ../..
```

Requirements: Python 3.12+, C compiler.
Optional: libuv (async/await), libcurl (fetch/HTTP), raylib (games/GUI).
...
```bash
tsuchi compile hello.js        # → ./hello
tsuchi compile app.ts          # TypeScript (types auto-stripped)
tsuchi compile app.js -o dist/ # Custom output directory
```

### Eval

```bash
tsuchi eval 'console.log(42 * 2)'
tsuchi eval --ts 'function add(a: number, b: number): number { return a+b; } console.log(add(1,2));'
echo 'console.log("hello")' | tsuchi eval
```
...
```bash
tsuchi compile app.js --backend quickjs   # Default (~1 MB, portable)
tsuchi compile app.js --backend jsc       # JavaScriptCore (~59 KB on macOS)
```
...
```bash
tsuchi compile examples/hello.js
tsuchi compile examples/fibonacci.js
```
...
```bash
tsuchi compile examples/neon_drive.js && ./neon_drive
```
...
```bash
tsuchi compile examples/clay_dashboard.jsx && ./clay_dashboard     # GUI
tsuchi compile examples/tui_filemanager.jsx --tui && ./tui_filemanager  # TUI
```
