...
```bash
tsuchi compile app.js --backend quickjs   # Default: QuickJS-NG (static linked, ~1 MB)
tsuchi compile app.js --backend jsc       # JavaScriptCore (system framework, ~59 KB on macOS)
```

### TypeScript

```bash
tsuchi compile app.ts        # Auto-strips types, uses annotations as inference hints
tsuchi eval --ts 'function add(a: number, b: number): number { return a+b; } console.log(add(1,2));'
tsuchi repl --ts             # TypeScript REPL mode
```
...
```bash
tsuchi compile <file>         # Compile to standalone binary
...
tsuchi check <file>           # Type-check only (no binary)

tsuchi eval 'code'            # Compile and run inline JS
  -e, --expression 'code'    # Alternative flag
  --ts                        # TypeScript mode
  echo 'code' | tsuchi eval  # Pipe support

tsuchi repl                   # Interactive REPL
  --ts                        # TypeScript mode
  --backend quickjs|jsc       # Runtime backend
```
