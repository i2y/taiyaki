...
from taiyaki_aot_compiler.codegen.backend_base import BackendBase, _PROJECT_ROOT
...
def _find_taiyaki_root() -> Path:
    """Locate the katana/taiyaki project root (two levels up from packages/taiyaki-aot-compiler/)."""
    # packages/taiyaki-aot-compiler/ -> packages/ -> katana/
    ...
    raise RuntimeError(
        ...
        "or ensure packages/taiyaki-aot-compiler/ is inside the katana monorepo."
    )
